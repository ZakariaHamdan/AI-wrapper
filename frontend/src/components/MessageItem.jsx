...
function MessageItem({ message }) {
  ...
  // PDF Export Function with Logo - Uses user question instead of SQL query
  const exportToPDF = () => {
    ...
    try {
        ...
        return new Promise((resolve, reject) => {
          ...
          img.onload = function() {
            try {
              ...
              doc.addImage(this, 'PNG', 14, 10, logoWidth, logoHeight);

              // Add title next to logo
              doc.setFontSize(16);
              doc.text('Database Query Results', 14 + logoWidth + 10, 20);

              // Add company info or subtitle (optional)
              doc.setFontSize(10);
              doc.setTextColor(100, 100, 100);
              doc.text('Generated by PHD - Database AI Agent', 14 + logoWidth + 10, 26);

              // Reset text color
              doc.setTextColor(0, 0, 0);

              resolve(Math.max(30, 10 + logoHeight + 5)); // Return Y position for next content
            } catch (error) {
          ...
          setTimeout(() => {
            if (!img.complete) {
              console.warn('Logo loading timeout, proceeding without it');
              doc.setFontSize(16);
              doc.text('Database Query Results', 14, 15);
              resolve(25);
            }
      ...
      // Add logo and header, then continue with the rest
      addLogoAndHeader().then((startY) => {
        ...
        if (userQuestion) {
          ...
          currentY += (questionLines.length * 4) + 10;
        }
